...
class _HomePageState extends State<HomePage> {
  ...
  //    // var rest = data['tickets'] as List;

  //     print("Heroku response Data $resp");
  //    // print('Rest: $rest');
  //    // ticket = rest.map<Tickets>((json) => Tickets.fromJson(json)).toList();
  //   }

  //   //ticketsFromJson(data);
  //   //print("Ticket size: ${resp.length}");
  //   print("Returngin $ticket");
  //   return ticket;
  // }

  Future<Response> pingHeroku() async {
    var testData = postToHerokuServerData();
    var testDataEnc = json.encode(testData);
    String url = "https://flyx-web-hosted.herokuapp.com/search";
    final response = await http.post(
                ...
                child: Column(
                  children: <Widget>[
                    Container(
                      height: height * .95,
                      width: width,
                      child: routeInformation(snapshot),
                    ),
                  ],
                ),
              );
  ...
  Container routeInformation(AsyncSnapshot<Response> snapshot) {
    return Container(
                    ...
                    child: Card(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, i) {
                          return Column(
                            children: <Widget>[
                              Text('from ${snapshot.data.data[index].route[i].cityFrom}' +
                                  ' to ${snapshot.data.data[index].route[i].cityTo}'),
                              Text(
...
Map<String, Object> postToHerokuServerData() {
  return {
    ...
    'radiusTo': 80.0,
    "departureWindow": {
      'start': "2019-04-16 10:27:41.589807",
